  ...
  return (
    <div className="container mx-auto">
      <div className="m-5 flex flex-col items-center justify-center">
        <h1 className="mb-10 text-4xl">React Read and Parse CSV Files</h1>
        ...
        {data.length > 0 && (
          <div className="relative mt-5 overflow-x-auto">
            <table className="w-full table-auto text-left text-sm text-slate-500 dark:text-slate-400">
              <thead className="bg-slate-200 text-xs uppercase text-slate-700 dark:bg-slate-700 dark:text-slate-400">
                <tr key="0">{renderRow(data[0], true)}</tr>
              </thead>
              <tbody>
                {data.map(
                  (item, index) => index !== 0 && <tr key={index}>{renderRow(item, false)}</tr>
                )}
              </tbody>
            </table>
          </div>
